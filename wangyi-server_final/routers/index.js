...
const indexCateModule = require('../data/indexCateModule.json')
// 热词搜索
const search1 = require('../data/search.json')
// 关键字搜索
const search2 = require('../data/search1.json')

// 分类列表数据接口
router.get('/getCateList', (req, res) => res.send({ code: 0, cateLists }))

// 分类列表导航栏数据接口
router.get('/getCateNav', (req, res) => res.send({ code: 0, cateNavDatas }))

// 商品列表数据接口
router.get('/getShopList', (req, res) => res.send({ code: 0, index }))

// 商品列表中的每一块数据接口
router.get('/getShopItem', (req, res) => res.send({ code: 0, indexCateModule }))

// 搜索列表接口1(热词搜索)
router.get('/search1', (req, res) => res.send({ code: 0, search1 }))

// 搜索列表接口2(关键字搜索)
router.get('/search2', (req, res) => res.send({ code: 0, search2 }))

// topic轮播图数据接口
router.get('/topicSwiper', (req, res) => res.se1nd({ code: 0, demo }))
/*
密码登陆
 */
...
router.post('/login_sms', function (req, res, next) {
  ...
  console.log(users, 333333)
  console.log('/login_sms', phone, code);
  console.log(users[phone], 2222)
  if (users[phone] != code) {
